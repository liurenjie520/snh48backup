		...
		return this.each(function() {
			var _SELF = $(this),
				...
				setSize = function(Dom,l,t,w,h,z,s){
					Dom.show()
						 .css({"z-index":z,"opacity":(w/options.width==1?1:(w/options.width*.8))})
				...
				object = function(length){
					...
					this.arrow = function(l,r,w,h,x,y){
						return "<em class=\"sPrev\" id=\"sPrev\" style=\"position:absolute;cursor:pointer;z-index:10;width:"+w+"px;height:"+h+"px;left:"+(x)+"px;top:"+y+"px;\"></em><em class=\"sNext\" id=\"sNext\" style=\"position:absolute;cursor:pointer;z-index:10;width:"+w+"px;height:"+h+"px;right:"+(x)+"px;top:"+y+"px;\"></em>";
					};
			...
			object.prototype.Initial = function(callback){
				...
				var a=null,b=null,c=null,sTmpDD=null,t=2,t2=this.centerCount+1,
				    ...
				    wrapL = $IMGWRAP.position().left,
					wrapR = $IMGWRAP.position().left,
					...
					for(c=0;c<options.count;c++){
						if(a===this.indexArr[c]){
							var zindex = a>CenIdxArr?a-t:a,
								...
								cha = tmp==1?0:tmp*.16,
								tmp_zoom = tmp*.55<1?1:tmp*.55,
								width = Math.round(options.width/tmp_zoom*options.setZoom),
								height = Math.round(options.height/tmp_zoom*options.setZoom),
								top = Math.round(setPos(options.position,height))
								left = Math.round(options.offsetX*tmp-(options.offsetX*tmp*cha));
							//console.log(tmp)
							if(a<CenIdxArr){
								t2-=1;
								left = left * -1;
			...
			var $prev = $("#sPrev",_SELF),
				...
				$dd = $("dd",_SELF);
			$prev.click(function(){
				if(_IDX == 1){//新加判断，向左到第二个子项后，跳到最后第二项 可以继续向左
					_IDX = imgLength-2
			...
			$next.click(function(){
				if(_IDX == imgLength-2){//新加判断，向又到倒数第二个子项后，跳到前面第二项 可以继续向右
					_IDX = 1
					imgScroll(_IDX,_arrData,$dd);
				}else if(_IDX < imgLength-1) {
					_IDX = _IDX + 1;
					imgScroll(_IDX,_arrData,$dd);
				};
				return false;
